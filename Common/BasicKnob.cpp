    ...
    Slider& slider)
{
    ...
    if (theSlider)
    {
        ...
        const float radius = jmin(width1 / 2, height1 / 2) - 10.0f;
        const float centreX = x + width1 * 0.5f;
        const float centreY = y + height1 * 0.5f;
        ...
        Image dialSwatch = ImageCache::getFromMemory (BinaryData::dialSwatch2_png, BinaryData::dialSwatch2_pngSize);
        
        Image scaledSwatch = dialSwatch.rescaled(width1, height1);
        g.drawImageAt(scaledSwatch, 0, 0);
        scaledSwatch.duplicateIfShared();
...
//        
//         Image dialKnob = ImageCache::getFromMemory (BinaryData::dialKnob_png, BinaryData::dialKnob_pngSize);
//        g.drawImage(dialKnob, p);
        // pointer
        Path p;
